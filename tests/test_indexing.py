...
class IndexingFallbackTests(unittest.TestCase):
    ...
    def test_index_adds_missing_tables_for_existing_db_without_schema_versioning(self) -> None:
        ...
        try:
            preview = conn.execute("SELECT preview_text FROM doc_previews").fetchone()
            semantic_meta = conn.execute("SELECT backend, model_name FROM semantic_meta WHERE singleton = 1").fetchone()
            self.assertIsNotNone(preview)
            self.assertIsNotNone(semantic_meta)
    ...
    def test_max_bytes_caps_text_content(self) -> None:
        ...
        try:
            ...
            chunk_rows = conn.execute("SELECT chunk_text FROM doc_chunks ORDER BY chunk_index").fetchall()
            self.assertEqual({"alpha": 1}, terms)
            self.assertEqual("alpha", preview)
            self.assertEqual([("alpha",)], chunk_rows)
        finally:
            conn.close()
...
class IndexCliHelpTests(unittest.TestCase):
    def test_help_lists_index_options_and_behavior(self) -> None:
        out = io.StringIO()
        with mock.patch.object(sys, "argv", ["pse_index.py", "--help"]):
            with redirect_stdout(out):
        ...
        self.assertIn("Usage: python pse_index.py index <root_folder>", output)
        self.assertIn("--help", output)
        self.assertIn(".pse_index.sqlite3", output)
        self.assertIn("pse_index.toml", output)

    def test_unknown_index_option_prints_help(self) -> None:
        out = io.StringIO()
        with mock.patch.object(sys, "argv", ["pse_index.py", "--bogus"]):
            with redirect_stdout(out):
                with self.assertRaises(SystemExit) as exc:
                    pse_index.main()
